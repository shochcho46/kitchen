 ...
 $headname = $this->db->select('*')->from('expense_item')->where('id',$id)->get()->row()->expense_item_name;
  ...
  public function headcode(){
     ...
     public function expense_item_insert(){
             ...
             $CreateBy=$this->session->userdata('id');
             $createdate=date('Y-m-d H:i:s');
			/*$newdata = $this->db->select('*')->from('acc_coa')->where('HeadName','Expence')->get()->row();
			$newidsinfo = $this->db->select('*')->from('acc_coa')->where('PHeadName',$newdata->HeadName)->order_by('HeadCode','desc')->get()->row();
            //echo $this->db->last_query();
			echo $nid  = $newidsinfo->HeadCode;
			$n =$nid + 1;
			if ($n / 10 < 1)
			  $headcode = $newdata->HeadCode . "0" . $n;
			else
			  $headcode = $newdata->HeadCode . $n;*/
			  $coa = $this->headcode();
			 $coa->HeadCode;
